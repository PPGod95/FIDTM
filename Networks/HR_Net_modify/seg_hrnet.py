    ...
    def init_weights(self, pretrained='', train=False):
        ...
        if train==True:
            if os.path.isfile(pretrained):
                ...
                logger.info('=> Train without pretrained model')
                # print('Train without pretrained model')
                # exit()


def get_seg_model(train=False):
    from Networks.HR_Net_modify.default import _C as hr_config
    from Networks.HR_Net_modify.default import update_config

    update_config(hr_config, './Networks/HR_Net_modify/seg_hrnet_w48.yaml')
    model = HighResolutionNet(hr_config)
    from Networks.HR_Net_modify.config import cfg
